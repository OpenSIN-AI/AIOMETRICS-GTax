...
async function loadConfig(): Promise<Config> {
  return {
    googleCredentialsPath: process.env.GOOGLE_CREDENTIALS_PATH || path.join(__dirname, '../../meine-google-credentials/credentials.json'),
...
async function createGoogleApis(credentialsPath: string): Promise<{ driveApi: drive_v3.Drive; sheetsApi: sheets_v4.Sheets }> {
  const auth = new JWT({
